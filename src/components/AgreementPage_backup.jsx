...
const AgreementPage = ({ agreementData: initialAgreementData, userType, agreementId, ideaId }) => {
  ...
  const generateDetailedAgreementPDF = () => {
    const doc = new jsPDF();
    const agreementText = `
      Agreement between ${agreementData.startupName} and ${agreementData.investorName}

      Investment Amount: ${agreementData.investmentAmount}
      Investment Range: ${agreementData.investmentRange}
      Preferred Industries: ${preferredIndustries}

      Startup Details:
      - Company Name: ${startupRegistrationData?.companyName || startupDetails?.companyName || 'N/A'}
      - Name: ${startupRegistrationData?.name || startupDetails?.name || 'N/A'}
      - Founder Name: ${startupRegistrationData?.founderName || startupDetails?.founderName || 'N/A'}
      - Idea Title: ${startupRegistrationData?.ideaTitle || startupDetails?.ideaTitle || 'N/A'}
      - Idea ID: ${startupRegistrationData?.ideaId || startupDetails?.ideaId || 'N/A'}
      - Funding Amount Requested: ${startupRegistrationData?.fundingAmountRequested || startupDetails?.fundingAmountRequested || 'N/A'}
      - Current Valuation: ${startupRegistrationData?.currentValuation || startupDetails?.currentValuation || 'N/A'}
      - Project Title: ${startupRegistrationData?.projectTitle || startupDetails?.projectTitle || 'N/A'}

      Investor Details:
      - Full Name: ${investorRegistrationData?.fullName || investorDetails?.fullName || 'N/A'}
      - Investor Name: ${investorRegistrationData?.investorName || investorDetails?.investorName || 'N/A'}
      - Investor Type: ${investorRegistrationData?.investorType || investorDetails?.investorType || 'N/A'}
      - Company Name: ${investorRegistrationData?.companyName || investorDetails?.companyName || 'N/A'}
      - Investment Range: ${investorRegistrationData?.investmentRange || investorDetails?.investmentRange || 'N/A'}
      - Preferred Industries: ${investorRegistrationData?.preferredIndustries || investorDetails?.preferredIndustries || 'N/A'}

      Signatures:
      - Startup: ${startupSignature || 'N/A'}
      - Investor: ${investorSignature || 'N/A'}
    ...
    doc.save('agreement.pdf');
      ...
      // Check by user name
      if (sig.userName === currentUserName) {
        console.log('Found current user signature by user name:', sig);
        return true;
      }
      
      // Check by specific signature field
      if (userType === 'startup' && agreementData.startupSignature) {
        console.log('Found current user signature by startup signature field');
        return true;
      }
      if (userType === 'investor' && agreementData.investorSignature) {
        console.log('Found current user signature by investor signature field');
        return true;
      }
    ...
    console.log('Current user signed result:', result);
    return result;
  };
